...
void init_screen(screen *s) {
    ...
    s->screen_cols = 0;
    s->screen_rows = 0;
    get_window_size(&s->screen_rows, &s->screen_cols);
}
...
void render_screen(document *d, editor *e, screen *s) { 
    ...
    for (int i = 0; i < s->screen_rows-1; i++) { 
        if (i + e->row_offset >= d->nlines) {
            ...
            write_to_buff(s, "\r", 1); 
            if (i < s->screen_rows - 1) write_to_buff(s, "\n", 1); 
        } 
    } 
    // ensures col snaps to new rows length if smaller then previous row 
    e->col = MIN((int)strlen(d->lines[e->row]), e->col); 
...
void render_status_bar(document *d, editor *e, screen *s) {
    ...
    s->size += snprintf(s->buff+s->size, s->cap-s->size, CURSOR_BOTTOM_ROW, s->screen_rows, 0);
    ...
    int remaining = s->screen_cols - (l_len + c_len + r_len);
